...
def start_director(tls=True):
    ...
    params = [
        "-c",
        str(director_config_path),
        "-rc",
        str(root_cert_path),
        "-pk",
        str(private_key_path),
        "-oc",
        str(public_cert_path),
    ]
    if not tls:
...
def stop_server(ca_server):
    subprocess.check_call(["fx", "pki", "uninstall", "-p", str(CA_PATH)])
...
def start_envoy(tls=False):
    ...
    with set_directory(envoy_folder):
        ...
        if not tls:
            params.append("--disable-tls")
        envoy = subprocess.Popen(["fx", "envoy", "start", *params])
        return envoy
