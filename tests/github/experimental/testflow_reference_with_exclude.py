...
class TestFlowReferenceWithExclude(FLSpec):
    ...
    def start(self):
        ...
        print(
            f"{bcolors.OKBLUE}Testing FederatedFlow - Starting Test for validating "
    ...
    def test_create_collab_attr(self):
        ...
        if len(TestFlowReferenceWithExclude.step_one_collab_attrs) >= MIN_COLLECTION_COUNT:
    ...
    def test_create_more_collab_attr(self):
        ...
        if len(TestFlowReferenceWithExclude.step_two_collab_attrs) >= MIN_COLLECTION_COUNT:
            collab_attr_list = filter_attrs(inspect.getmembers(self))
            matched_ref_dict = find_matched_references(
                collab_attr_list,
    ...
    def end(self):
        print(
            f"{bcolors.OKBLUE}Testing FederatedFlow - Ending test for validatng the "
            + f"references. {bcolors.ENDC}"
        )
    ...
    if collborators_sharing_ref:
        for collab in collborators_sharing_ref:
            if collab not in TestFlowReferenceWithExclude.all_ref_error_dict:
                TestFlowReferenceWithExclude.all_ref_error_dict[collab] = matched_ref_dict.get(
                    collab
                )
