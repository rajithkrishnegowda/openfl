...
def test_compress(tensor_key, named_tensor):
    ...
    (
        ...
        compressed_nparray,
        metadata,
    ) = tensor_codec.compress(tensor_key, nparray)
...
def test_compress_lossless(tensor_key, named_tensor):
    ...
    (
        ...
        compressed_nparray,
        metadata,
    ) = tensor_codec.compress(tensor_key, nparray, require_lossless=True)
...
def test_find_dependencies_without_send_model_deltas(tensor_key):
    ...
    tensor_codec = TensorCodec(NoCompressionPipeline())
    tensor_name, origin, round_number, report, tags = tensor_key
    tensor_key = TensorKey(tensor_name, origin, 5, report, ("model",))
...
def test_find_dependencies(tensor_key):
    ...
    assert tensor_key_dependency_1.tags == (
        "aggregated",
        "delta",
        "compressed",
    )
